...
def predict_classification():
    # Получение данных из запроса http://localhost:5000/api_class?height=175&weight=68&foot=44
    input_data = np.array([[float(request.args.get('height')),
                    float(request.args.get('weight')),
                    float(request.args.get('foot'))]])
    print(input_data)
    # input_data = np.array(input_data.reshape(-1, 1))
    ...
    print(predictions)
    result = 'Girl' if predictions >= 0.5 else 'Man'
    # print(result)
    # меняем кодировку
